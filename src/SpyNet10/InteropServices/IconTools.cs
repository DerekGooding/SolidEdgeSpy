...
public static class IconTools
{
    ...
    public static Icon GetIconForExtension(string extension, ShellIconSize size)
    {
        ...
        using (var keyForExt = Registry.ClassesRoot.OpenSubKey(extension))
        {
            ...
            using (var keyForClass = Registry.ClassesRoot.OpenSubKey(className))
            {
                ...
                if (keyForIcon == null)
                {
                    ...
                    var clsid = "CLSID\\" + Convert.ToString(keyForCLSID.GetValue(null));
